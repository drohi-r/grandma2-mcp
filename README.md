---
title: MA2 Agent
description: AI agent for grandMA2 lighting consoles — 199 MCP tools via Telnet
version: 1.0.0
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/MCP_Tools-199-FF2020?style=for-the-badge" alt="199 MCP Tools">
...
**An AI agent for grandMA2 lighting consoles.** Exposes 199 grandMA2 operations as [Model Context Protocol](https://modelcontextprotocol.io/) tools so AI assistants (Claude Desktop, VS Code, etc.) can drive a lighting console via Telnet. Includes a built-in orchestrator, task decomposer, and long-term memory for fully autonomous lighting control.

<table>
<tr><td><b>Agent Harness</b></td><td>199 MCP tools covering every grandMA2 operation — playback, programming, user management, show files, busking, and more. Connect any MCP-compatible AI assistant and start controlling the console immediately.</td></tr>
...
[Quick Start](#quick-start) · [Architecture](#architecture) · [199 MCP Tools](#mcp-tools) · [Resources](#mcp-resources) · [Prompts](#mcp-prompts) · [Skills](#agent-skills) · [Safety System](#safety-system) · [RAG Pipeline](#rag-pipeline)
...
graph TD
    ...
    A["🎭 MCP Server Layer<br/><code>src/server.py</code><br/>199 tools · safety gate"] --> B
...
|-------|-----------|-----------|
| **Bottom 199 tools** | **Agent Harness** — exposes a tool surface to an external AI; the reasoning loop lives in Claude Desktop, VS Code, etc. | `src/server.py` |
...
| Module | Role |
|--------|------|
| `src/server.py` | FastMCP server, 165 interactive tools, safety gate, env config |
...
The server exposes **199 tools** to MCP clients, grouped into 15 categories plus an agentic orchestration layer:
...
| `ma2://docs/tool-taxonomy` | ML-clustered tool taxonomy — 165 base tools clustered into 14 categories (JSON) |
...
All 199 tools are mapped in `doc/ma2-rights-matrix.json`.
...
ma2-onPC-MCP/
├── src/
│   ├── server.py                           # FastMCP server (165 interactive tools)
